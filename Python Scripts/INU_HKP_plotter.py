...
states = ['LS_ASW_TRX_1','LS_ASW_TRX_2']#df['COND_LOGIC_STATE'].unique().tolist()
show_states = ['_'.join(el.split('_')[2:]) for el in states]#[el.split('_')[4] for el in states]
show_states = dict(zip(range(1,len(show_states)+1),show_states))
states = dict(zip(range(1,len(states)+1),states))
